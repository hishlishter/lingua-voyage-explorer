...
-- Создание тестового пользователя с известными данными для входа
-- ВАЖНО: Это только для демонстрации, в реальных проектах нельзя хранить пароли в открытом виде
-- Эта запись создает пользователя с email: test@example.com и паролем: testpassword123

-- Пользователь уже создается через функцию signInWithTestAccount в AuthContext.tsx, 
-- поэтому здесь не требуются дополнительные команды для создания пользователя
